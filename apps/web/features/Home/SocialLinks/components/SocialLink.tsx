...
function SocialButton({
  ...
  return (
    ...
    >
      {/* Main button */}
      <div
        className={`relative w-12 h-12 rounded-xl ${colorClass} shadow-md flex items-center justify-center transition-all duration-300 group-hover:shadow-lg`}
      >
        {/* Icon container */}
        <div className="relative w-6 h-6 text-white">{item.icon}</div>
      </div>
    ...
    <div className="relative">
      ...
      <div className="relative flex justify-center items-center gap-4 p-6">
        {config.map((item, index) => (
          <SocialButton key={item.value} item={item} index={index} />
        ))}
      </div>
    </div>
  )
